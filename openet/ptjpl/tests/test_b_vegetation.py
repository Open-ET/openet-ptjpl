...
def test_savi_from_ndvi(ndvi=0.5, expected=0.5 * 0.45 + 0.132, tol=0.000001):
    output = utils.constant_image_value(vegetation.savi_from_ndvi(
...
def test_fAPAR_from_savi(savi, expected, tol=0.000001):
    output = utils.constant_image_value(vegetation.fAPAR_from_savi(
        ee.Image.constant(savi)))
    assert abs(output - expected) <= tol
...
def test_fAPAR_from_ndvi(ndvi, expected, tol=0.000001):
    output = utils.constant_image_value(vegetation.fAPAR_from_ndvi(
        ee.Image.constant(ndvi)))
    assert abs(output - expected) <= tol
...
def test_fIPAR_from_ndvi(ndvi, expected, tol=0.000001):
    output = utils.constant_image_value(vegetation.fIPAR_from_ndvi(
        ee.Image.constant(ndvi)))
    assert abs(output - expected) <= tol
